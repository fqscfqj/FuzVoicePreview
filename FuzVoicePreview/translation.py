...
try:  # pragma: no cover - exercised only inside MO2 / PyQt6 runtime
    ...
    def _candidate_translation_paths(base_name: str, translation_dirs: list[Path], ui_languages: list[str]) -> list[Path]:
        candidates: list[Path] = []
        seen: set[Path] = set()
        for translation_dir in translation_dirs:
            for language in ui_languages:
                for variant in _normalize_language_tag(language):
                    path = translation_dir / f"{base_name}_{variant}.qm"
                    if path in seen:
                        continue
                    seen.add(path)
    ...
    def _install_translator() -> None:
        ...
        translation_dirs = [plugin_dir / "i18n", plugin_dir]
        ...
        for candidate in _candidate_translation_paths(base_name, translation_dirs, ui_languages):
        ...
        for translation_dir in translation_dirs:
            if translator.load(locale, base_name, "_", str(translation_dir)):
                app.installTranslator(translator)
                _TRANSLATOR = translator
                return
